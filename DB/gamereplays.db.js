...
gamereplays.getList = function(){
  ...
  return this.coll.find({}).project({ name: 1, winner: 1, reward: 1}).toArray()
  .then((res_replaylist)=>{
...
gamereplays.getNewRoundPositions = function(replay_id, round_ix, player_name){
  ...
  return this.coll.findOne({ _id: ObjectId(replay_id)}, {projection: {rounds: { $slice: [round_ix, 1]}, "rounds.new_round_positions": 1}})
  .then((res)=>{
...
gamereplays.getInputsForRound = function(replay_id, round_ix){
  ...
  console.log("get inputs for round" + round_ix );

  return this.coll.findOne({ _id: ObjectId(replay_id)}, { projection: {rounds: {$slice:[round_ix,1]}, "rounds.inputs": 1} } )
  .then((res)=>{
